...
// Tracer (bullet) skins — colour your shots for everyone to see.
const TRACERS = {
  standard: { name: 'STANDARD', color: 0xffd080, tier: 0, cost: 0 },
  ice:      { name: 'ICE', color: 0x9fe0ff, tier: 3, cost: 0 },
  emerald:  { name: 'EMERALD', color: 0x6bffa0, tier: 8, cost: 0 },
  crimson:  { name: 'CRIMSON', color: 0xff5a5a, tier: 14, cost: 0 },
  amethyst: { name: 'AMETHYST', color: 0xb98bff, tier: 22, cost: 0 },
  gold:     { name: 'GOLD', color: 0xffd24a, tier: 0, cost: 500 },
  plasma:   { name: 'PLASMA', color: 0x35f0ff, tier: 0, cost: 900 },
  neon:     { name: 'NEON PINK', color: 0xff4fd8, tier: 0, cost: 1300 },
  voidfx:   { name: 'VOID', color: 0x8a4bff, tier: 0, cost: 2000 },
  rainbow:  { name: 'RAINBOW', color: 0xffffff, tier: 0, cost: 3500, rainbow: true },
};
let ownedTracers = (() => { try { const a = JSON.parse(localStorage.getItem('bf_tracers')); return new Set(Array.isArray(a) ? a : []); } catch { return new Set(); } })();
ownedTracers.add('standard');
let equippedTracer = localStorage.getItem('bf_tracer') || 'standard';
function saveTracers() { localStorage.setItem('bf_tracers', JSON.stringify([...ownedTracers])); localStorage.setItem('bf_tracer', equippedTracer); }
function tracerUnlocked(id) { const t = TRACERS[id]; if (!t) return false; if (ownedTracers.has(id)) return true; return t.tier > 0 && careerLevel() >= t.tier; }
// Effective tracer colour for one of MY shots (rainbow cycles over time).
function myTracerColor(wkey) {
  const t = TRACERS[equippedTracer];
  if (!t || equippedTracer === 'standard') return wkey === 'sniper' ? 0xaadfff : 0xffd080;
  if (t.rainbow) { const c = new THREE.Color(); c.setHSL((performance.now() * 0.0004) % 1, 1, 0.6); return c.getHex(); }
  return t.color;
}
...
// Cosmetics shop with two categories: titles + tracer skins.
let shopCat = 'titles';
...
function renderShop() {
  ...
  if (shopCat === 'titles') {
    ...
    for (const id in TRACERS) {
      const t = TRACERS[id], owned = tracerUnlocked(id), equipped = equippedTracer === id;
      const swatch = t.rainbow ? 'background:linear-gradient(90deg,#ff5a5a,#ffd24a,#6bffa0,#35f0ff,#b98bff)'
        : `background:#${t.color.toString(16).padStart(6, '0')}`;
      h += `<div class="shop-card ${owned ? 'owned' : ''} ${equipped ? 'equipped' : ''}">
        <div class="tracer-sw" style="${swatch}"></div>
        <div class="shop-title">${t.name}</div>
        <div class="shop-src">${t.tier > 0 ? 'Career reward' : id === 'standard' ? 'Default' : 'Shop exclusive'}</div>
        ${actionBtn(owned, equipped, t.tier, t.cost, id)}</div>`;
    }
  }
  box.innerHTML = h;
  box.querySelectorAll('[data-equip]').forEach(b => b.addEventListener('click', () => shopCat === 'titles' ? equipTitle(b.dataset.equip) : equipTracer(b.dataset.equip)));
  box.querySelectorAll('[data-buy]').forEach(b => b.addEventListener('click', () => shopCat === 'titles' ? buyTitle(b.dataset.buy) : buyTracer(b.dataset.buy)));
  const none = $('shopUnequip'); if (none) { none.style.display = shopCat === 'titles' ? '' : 'none'; none.onclick = () => equipTitle('none'); }
}
function equipTracer(id) { if (!tracerUnlocked(id)) return; equippedTracer = id; saveTracers(); renderShop(); }
function buyTracer(id) {
  const t = TRACERS[id]; if (!t || tracerUnlocked(id)) return;
  if (coins < t.cost) { const m = $('shopMsg'); if (m) m.textContent = `Need ${t.cost} coins`; return; }
  awardCoins(-t.cost); ownedTracers.add(id); saveTracers(); equipTracer(id);
  const m = $('shopMsg'); if (m) m.textContent = `Unlocked ${t.name} tracer!`;
}
