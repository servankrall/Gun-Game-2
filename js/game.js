...
const SKINS = {
  ...
  abyss:       { name: 'ABYSSAL',      body: 0x061420, accent: 0x1fd0e0, emissive: 0x083040, tracer: 0x2affea, tier: 0, cost: 2000 },
  sakura:      { name: 'SAKURA',       body: 0x2a1220, accent: 0xff8ac0, emissive: 0x3a1030, tracer: 0xff9ad0, tier: 0, cost: 1600 },
  bumblebee:   { name: 'BUMBLEBEE',    body: 0x1a1606, accent: 0xffd21a, emissive: 0x2a2200, tracer: 0xffe04a, tier: 0, cost: 1400 },
  wraith:      { name: 'WRAITH',       body: 0x0c0c12, accent: 0x7a3bff, emissive: 0x1a0a3a, tracer: 0xb04fff, tier: 0, cost: 2800 },
  goldrush:    { name: 'GOLD RUSH',    body: 0x3a2c08, accent: 0xffe066, emissive: 0x4a3a10, tracer: 0xffe066, tier: 0, cost: 3600 },
};
...
const SKIN_PATTERN = {
  ...
  abyss: 'wave', sakura: 'scale', bumblebee: 'hex', wraith: 'glitch', goldrush: 'gold',
};
// Premium skins pulse an animated glow in-game and in the preview.
const GLOW_SKINS = new Set(['chronovoid', 'radiant', 'neofrontier', 'oni', 'glitchpop', 'dragon', 'toxic', 'molten', 'cyber', 'mythicgun', 'abyss', 'wraith', 'goldrush']);
const _skinTexCache = {};
...
function skinRarity(id) {
  ...
  if (id === 'dragon' || id === 'mythicgun' || id === 'goldrush' || id === 'wraith') return { name: 'EXOTIC', color: '#ff7a1a' };
  if (s.tier > 0) return { name: 'CAREER', color: '#7be0a0' };
...
const tracers = [], particles = [], flashes = [], rockets = [], grenades = [], dmgTexts = [], killFx = [];
let camShake = 0; // screen-shake magnitude, decays each frame
let _ringTex = null;
function ringTexture() {
  if (_ringTex) return _ringTex;
  const c = document.createElement('canvas'); c.width = c.height = 64;
  const g = c.getContext('2d');
  g.strokeStyle = '#fff'; g.lineWidth = 6; g.beginPath(); g.arc(32, 32, 26, 0, Math.PI * 2); g.stroke();
  g.lineWidth = 2; g.globalAlpha = 0.6; g.beginPath(); g.arc(32, 32, 15, 0, Math.PI * 2); g.stroke();
  _ringTex = new THREE.CanvasTexture(c); return _ringTex;
}
...
// A juicy elimination burst: expanding shockwave ring + skull-coloured fountain
// + a floating ELIMINATED banner, all tinted by your skin colour.
function makeKillEffect(pos, colorHex, big) {
  burst(pos, colorHex, big ? 46 : 30, big ? 9 : 7);
  burst(pos, 0xffffff, 10, 4);
  const ring = new THREE.Sprite(new THREE.SpriteMaterial({ map: ringTexture(), color: colorHex, transparent: true, opacity: 0.95, blending: THREE.AdditiveBlending, depthTest: false }));
  ring.position.copy(pos); ring.scale.set(0.5, 0.5, 0.5);
  scene.add(ring); killFx.push({ obj: ring, ttl: 0.5, max: 0.5, kind: 'ring' });
  const sp = textSprite(big ? 'ELIMINATED!' : 'ELIMINATED', '#' + (colorHex >>> 0).toString(16).padStart(6, '0'));
  sp.position.copy(pos); sp.position.y += 0.5;
  scene.add(sp); killFx.push({ obj: sp, ttl: 1.2, max: 1.2, kind: 'text' });
}
...
function leaveGame() {
  ...
  leavingGame = true;
  if (ws && (ws.readyState === 0 || ws.readyState === 1)) { try { ws.close(); } catch {} }
  // Always return to the menu, even if onclose is slow or never fires.
  setTimeout(() => { if (inGame) backToMenu(); leavingGame = false; }, 250);
  if (!ws || ws.readyState > 1) backToMenu();
}
...
function connect(name) {
  ...
  ws.onclose = () => {
    if (inGame) {
      ...
      if (!leavingGame) $('menuErr').textContent = 'Connection lost. Join again!'; // only on a real drop
      leavingGame = false;
      backToMenu();
    }
  };
  ...
  switch (m.t) {
    ...
    case 'death': {
      ...
      if (m.killer === myId && m.victim !== myId) {
        ...
        camShake = Math.min(0.5, 0.16 + (multiKill >= 2 ? 0.12 * multiKill : 0));
        const kf = $('killFlash'); if (kf) { kf.style.color = hx(kcol); kf.style.opacity = multiKill >= 2 ? '0.6' : '0.4'; clearTimeout(kf._t); kf._t = setTimeout(() => kf.style.opacity = '0', 110); }
...
function updateFx(dt) {
  ...
  for (let i = killFx.length - 1; i >= 0; i--) {
    const f = killFx[i]; f.ttl -= dt;
    const k = 1 - Math.max(0, f.ttl) / f.max;
    if (f.kind === 'ring') { const s = 0.5 + k * 4.0; f.obj.scale.set(s, s, s); f.obj.material.opacity = (1 - k) * 0.95; }
    else { f.obj.position.y += dt * 0.9; f.obj.material.opacity = Math.min(1, f.ttl / 0.4); }
    if (f.ttl <= 0) { scene.remove(f.obj); f.obj.material.map && f.obj.material.map.dispose(); f.obj.material.dispose(); killFx.splice(i, 1); }
  }
...
function updateViewModel(dt, moving) {
  ...
  vm.group.rotation.set(vm.recoil * 0.14, 0, 0); // reset y/z each frame so inspect offsets don't accumulate (fixes crooked gun)
